...
Scope (MERGE-PLAN §2.4 / §8):
- This proves the *replay logic* is correct, NOT that the executor is honest
  (that needs Check 1 / GPU, §8 U8), and NOT that it accepts a real v011
  executor artifact (the production weight path is not yet reproducible,
  §8 U11/U12). The artifact here is self-consistent: the "reported" token is
  computed by the same decimal pipeline the validator replays.
"""
