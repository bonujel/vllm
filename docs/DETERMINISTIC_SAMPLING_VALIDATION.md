...
> **Errata / implementation notes (branch `tg/deterministic_sampling_merged`).**
> This doc was ported from the `dump` branch. Where the merged branch's code
> deviates or where the design has open blockers, see
> [`MERGE-PLAN-deterministic-sampling.md`](../MERGE-PLAN-deterministic-sampling.md)
> §8 and [`docs/adr/`](adr/):
> - **Seed** is `{user_seed}|{prompt_token_ids}` (§176/§498). This is
>   request-controlled and grindable — §8 U1 (blocker, not fixed here).
> - **`decimal` context** is applied *locally* (`localcontext()`), not
>   module-globally as this doc's snippet shows — ADR 0002 / §8 U10.
> - **`verify_sampling_from_logprobs`** (§415) is implemented per this doc's
>   contract (single position, `-> bool`, takes `seed_str`); it takes
>   `logprobs: Dict[str, float]` and converts via `repr(f)` internally — ADR
>   0001 / §8 U9. Sequence-level looping + `serving_chat.py` wiring are NOT
>   done here — §8 U6.
> - **Executor production path** still derives weights from GPU-float probs,
>   not the decimal pipeline — so Check 2 does NOT yet agree with real executor
>   artifacts. Blockers §8 U11 (weight source) and U12 (weight-list ordering).
> - Only **Check 2** ships here (pure CPU). **Check 1** (GPU model re-run) is
>   out of scope — §8 U8.
